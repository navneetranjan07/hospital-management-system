...
import { FaEnvelope, FaKey, FaSpinner, FaStethoscope, FaHospital, FaHeart, FaInstagram, FaGithub, FaTwitter, FaLinkedin } from "react-icons/fa";
      ...
      <div className="flex-1 flex items-center justify-center p-4 md:p-6 lg:p-8">
        <div className="w-full max-w-md bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-4 md:p-6 animate-fadeIn hover:shadow-3xl transition-all duration-500">
          ...
          <div className="text-center mb-4 md:mb-6">
            <h2 className="text-2xl md:text-3xl font-bold text-white mb-2 bg-gradient-to-r from-cyan-300 to-blue-300 bg-clip-text text-transparent">Email Verification</h2>
            <p className="text-gray-200 text-sm md:text-base font-medium">Verify your email to continue.</p>
          </div>
            ...
            <p
              className={`text-center mb-3 py-2 rounded-lg ${
                message.includes("success") || message.includes("verified")
                  ? "text-green-400 bg-green-900/20 border border-green-500/30 shadow-sm animate-pulse"
                  : "text-red-400 bg-red-900/20 border border-red-500/30 shadow-sm animate-bounce"
              }`}
            >
              {message}
            </p>
          ...
          {!otpSent ? (
            ...
            <form onSubmit={handleVerifyOtp} className="space-y-4">
              <div className="relative group">
                <FaKey className="absolute top-3.5 left-3.5 text-gray-400 group-focus-within:text-cyan-300 transition-all duration-300" />
                <input
                  type="text"
